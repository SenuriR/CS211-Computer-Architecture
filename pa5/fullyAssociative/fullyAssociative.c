...
void accessData (
    ...
    // dequeue from front of FIFO, update occupancy, and record an eviction
        ...
        // save the front of the cache
        cache_line_t* tmp = cache->front;

        // take the front of the cache off
    ...
    cache_line_t* new_cache_line = malloc(sizeof(cache_line_t));
    new_cache_line->tag = tag;
    new_cache_line->next_cache_line = NULL;

    // if there is a back, connect that back to this new node
    if (cache->back != NULL){
        cache->back->next_cache_line = new_cache_line;
    }
    cache->back = new_cache_line;

    // make sure the front makes sense
    if(cache->front == NULL){
        cache->front = new_cache_line;
    }
    cache->occupancy++;
